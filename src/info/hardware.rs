...
pub fn get_battery_info(_components: &Components) -> String {
    #[cfg(target_os = "linux")]
    {
        ...
        if let Ok(entries) = fs::read_dir(batt_dir) {
            ...
            for entry in entries.flatten() {
                ...
                let base = entry.path();
                if let Ok(cap) = fs::read_to_string(base.join(BATT_CAPACITY)) {
                    if let Ok(pct) = cap.trim().parse::<u32>() {
                        total_pct += pct;
                        batt_count += 1;
                    }
                }
                if let Ok(s) = fs::read_to_string(base.join(BATT_STATUS)) {
    ...
    if cfg!(target_os = "macos") {
        if let Ok(output) = Command::new(PMSET_CMD).args(["-g", "batt"]).output() {
            let out = String::from_utf8_lossy(&output.stdout);
